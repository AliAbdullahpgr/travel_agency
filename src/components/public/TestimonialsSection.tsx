    ...
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }, (_, i) => (
        <svg
          key={i}
          viewBox="0 0 20 20"
          className={`h-3.5 w-3.5 ${i < count ? "text-[color:var(--gold)]" : "text-white/10"}`}
          fill="currentColor"
        >
...
const TimelineCard = ({
  ...
  return (
    ...
    >
      <div
        className={`absolute top-6 hidden h-4 w-4 rounded-full border-[3px] border-[color:var(--gold)] bg-(--emerald-deep) shadow-[0_0_15px_rgba(212,175,55,0.4)] md:block ${
      ...
      <div className="group relative w-full overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.05] to-white/[0.01] p-6 backdrop-blur-md transition-all duration-500 hover:border-[color:var(--gold)]/30 hover:bg-white/[0.07]">
        ...
        <div className="relative z-10">
          ...
          <blockquote className="mb-6">
            <p className="font-serif text-lg leading-relaxed text-white/90">{item.quote}</p>
          </blockquote>

          <div className="flex items-center gap-4 border-t border-white/10 pt-4">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[color:var(--gold)] to-[color:var(--gold-dim)] font-bold text-[#051a17] shadow-lg">
              {item.name
                .split(" ")
                .map((n) => n[0])
                .join("")}
            ...
            <div>
              <p className="font-display text-sm font-bold tracking-wide text-white">{item.name}</p>
...
export function TestimonialsSection({
  ...
  return (
    <section ref={containerRef} className="relative z-10 overflow-hidden bg-(--emerald-deep) px-4 py-24 md:py-32">
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mb-20 text-center md:mb-32">
          ...
          >
            ...
            <h2 className="font-display px-4 text-4xl leading-tight text-white md:text-6xl lg:text-7xl">
              {title} <span className="text-white/50">{titleAccent}</span>
            </h2>
          </motion.div>
        </div>
        ...
        <motion.div style={{ y: yStats }} className="mt-20 border-t border-white/10 pt-16 md:mt-32">
