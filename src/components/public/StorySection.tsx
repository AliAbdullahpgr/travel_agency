...
export function StorySection({ eyebrow, title, intro, items }: StorySectionProps) {
  ...
  return (
    <section ref={sectionRef} className="relative mx-auto max-w-7xl overflow-hidden px-4 py-16 md:px-8 md:py-24">
      ...
      <div
        data-parallax="0.12"
        className="pointer-events-none absolute inset-[-15%_0] z-0 opacity-[0.06]"
        style={{
          backgroundImage:
      ...
      <div className="relative z-10">
        <div data-reveal className="max-w-3xl space-y-4">
          <p className="inline-flex items-center gap-2 text-[0.65rem] font-bold tracking-[0.26em] uppercase text-[color:var(--gold-light)]">
            <span className="inline-block h-px w-6 bg-gradient-to-r from-[color:var(--gold)] to-transparent" />
            {resolvedEyebrow}
          </p>
          <h2 className="font-display text-3xl leading-tight text-white whitespace-pre-line md:text-5xl">
            {resolvedTitle}
          </h2>
          ...
          {resolvedItems.map((item, index) => {
            ...
            return (
              <article
                key={item.title}
                data-reveal
                className="group relative overflow-hidden rounded-[1.8rem] border border-white/[0.08] bg-[linear-gradient(145deg,rgba(255,255,255,0.05),rgba(255,255,255,0.015))] p-6 transition-all duration-350 hover:border-[color:var(--gold)]/25 hover:-translate-y-1"
              >
                {/* Glow */}
                <div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-[color:var(--gold)]/[0.15] blur-[2rem] transition-transform duration-500 group-hover:scale-150" />

                {/* Icon */}
                {Icon && (
                  <div className="relative mb-4 flex h-12 w-12 items-center justify-center rounded-xl border border-[color:var(--gold)]/20 bg-[linear-gradient(135deg,rgba(200,168,78,0.15),rgba(200,168,78,0.06))]">
                    <Icon className="h-5 w-5 text-[color:var(--gold-light)]" />
                  </div>
                )}

                <p className="relative text-[0.6rem] font-bold tracking-[0.2em] uppercase text-[color:var(--gold-light)]">
                  {`0${index + 1}`}
                </p>
                <h3 className="relative mt-2 font-display text-xl text-white md:text-2xl">
                  {item.title}
                </h3>
