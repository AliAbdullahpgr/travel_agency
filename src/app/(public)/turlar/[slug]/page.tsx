    ...
    <>
      <section className="relative overflow-hidden pb-16">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-6 pt-20 md:px-8 md:pb-10 md:pt-24">
          ...
          <div className="mt-6 max-w-4xl">
            <p className="text-xs font-semibold tracking-[0.25em] uppercase text-(--gold-light)">Program Detayı</p>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            ...
            <span className="rounded-full border border-white/20 bg-black/30 px-4 py-2 text-sm text-(--gold-light)">
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-4 py-16 md:grid-cols-[1.3fr_0.7fr] md:px-8">
        <article className="space-y-6">
          <header>
            <h2 className="font-display text-3xl text-[color:var(--emerald)] md:text-4xl">Program Akışı</h2>
            <p className="mt-2 text-sm text-gray-500">Planlanan günlük ritim aşağıdaki gibidir.</p>
          </header>
          ...
          <div className="space-y-3">
            {tour.itinerary.map((item) => (
              <div
                ...
                className="rounded-2xl border border-gray-100 bg-white p-4 shadow-[0_2px_16px_rgba(0,0,0,0.04)]"
              >
                <p className="text-xs font-semibold tracking-[0.16em] uppercase text-[color:var(--gold)]">{item.day}</p>
                <h3 className="mt-2 font-display text-2xl text-gray-900">{item.title}</h3>
        ...
        <aside className="space-y-5">
          <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
            <h3 className="font-display text-2xl text-[color:var(--emerald)]">Paket Dahil</h3>
          ...
          <section className="rounded-2xl border border-red-200 bg-red-50 p-5">
            <h3 className="font-display text-2xl text-red-700">Paket Dışı</h3>
            <ul className="mt-4 space-y-2">
              {tour.excludes.map((item) => (
                <li key={item} className="flex items-start gap-2 text-sm text-gray-600">
                  <FiMinusCircle className="mt-0.5 h-4 w-4 shrink-0 text-red-400" />
                  {item}
                </li>
              ))}
            </ul>
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 md:px-8">
        <h2 className="font-display text-3xl text-[color:var(--emerald)] md:text-4xl">Sık Sorulan Sorular</h2>
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {tour.faq.map((item) => (
            <article key={item.question} className="rounded-2xl border border-gray-100 bg-white p-5 shadow-[0_2px_16px_rgba(0,0,0,0.04)]">
